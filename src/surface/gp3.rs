...
pub struct GreedyProjectionTriangulation<PointInT: kd_tree::KdPoint> {
    ...
    uvn_sfn: Vector2<f64>,
    // 下一个点的第一个边缘邻居的 2D 坐标
    uvn_next_ffn: Vector2<f64>,
    // 下一个点的第二个边缘邻居的 2D 坐标
    uvn_next_sfn: Vector2<f64>,

    // 临时变量，用于存储 3 个坐标
    tmp: Vector3<f64>,

    // 输入点云
    input: Option<PointCloud<PointInT>>,
    // 点索引
    indices: Option<Vec<usize>>,
    // KD 树
    kdtree: KdTreeRust<PointInT>,
}
/* end 结构体 */
